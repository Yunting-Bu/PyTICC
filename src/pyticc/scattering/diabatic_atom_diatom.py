...
def build_hamiltonian(
    ...
    *,
    ...
    def V_blocks_device(radial_points: NDArray[np.float64], channel_blocks: tuple[tuple[int, ...], ...], device: JaxDevice) -> tuple[jax.Array, ...]:
        """Sample the PES on CPU and contract requested channel blocks on a JAX device."""
        key = (device.platform, device.id)
        if key not in device_bases:
            device_bases[key] = vmat.device_basis(V_basis, device)
        potential_grid = vmat.sample(pes, radial_points, V_basis)
        return tuple(vmat.contract_device(V_basis, device_bases[key], potential_grid, device, indices) for indices in channel_blocks)
